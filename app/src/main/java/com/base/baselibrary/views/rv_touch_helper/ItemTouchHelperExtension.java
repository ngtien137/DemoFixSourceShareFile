...
public class ItemTouchHelperExtension extends RecyclerView.ItemDecoration
    ...
    private final RecyclerView.OnItemTouchListener mOnItemTouchListener
        ...
        public boolean onInterceptTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            if (action == MotionEvent.ACTION_DOWN) {
                mActivePointerId = event.getPointerId(0);//MotionEventCompat.getPointerId(event, 0);
                ...
                // can select the item
                final int index = event.findPointerIndex(mActivePointerId);//MotionEventCompat.findPointerIndex(event, mActivePointerId);
                if (index >= 0) {
        ...
        public void onTouchEvent(RecyclerView recyclerView, MotionEvent event) {
            ...
            final int action = event.getActionMasked();//MotionEventCompat.getActionMasked(event);
            final int activePointerIndex = event.findPointerIndex(mActivePointerId);//MotionEventCompat.findPointerIndex(event, mActivePointerId);
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_POINTER_UP: {
                    mClick = false;
                    final int pointerIndex = event.getActionIndex();//MotionEventCompat.getActionIndex(event);
                    final int pointerId = event.getPointerId(pointerIndex);//MotionEventCompat.getPointerId(event, pointerIndex);
                    if (pointerId == mActivePointerId) {
                        // This was our active pointer going up. Choose a new
                        // active pointer and adjust accordingly.
                        final int newPointerIndex = pointerIndex == 0 ? 1 : 0;
                        mActivePointerId = event.getPointerId(newPointerIndex);//MotionEventCompat.getPointerId(event, newPointerIndex);
    ...
    private float getSwipeWidth() {
        if (mSelected instanceof ITouchHelperExtension) {
            return ((ITouchHelperExtension) mSelected).getLayoutMenuWidth();
        }
    ...
    private boolean checkSelectForSwipe(int action, MotionEvent motionEvent, int pointerIndex) {
        ...
        mDx = mDy = 0f;
        mActivePointerId = motionEvent.getPointerId(0);//MotionEventCompat.getPointerId(motionEvent, 0);
...
//                        return;
//                    }
//                    int pointerId = e.getPointerId(0);//MotionEventCompat.getPointerId(e, 0);
//                    // Long press is deferred.
//                    // Check w/ active pointer id to avoid selecting after motion
//                    // event is canceled.
//                    if (pointerId == mActivePointerId) {
//                        final int index = e.findPointerIndex(mActivePointerId);//MotionEventCompat.findPointerIndex(e, mActivePointerId);
//                        final float x = e.getX(index);//MotionEventCompat.getX(e, index);
//                        final float y = e.getY(index);//MotionEventCompat.getY(e, index);
//                        mInitialTouchX = x;
//                        mInitialTouchY = y;
//                        mDx = mDy = 0f;
//                        loge("onlong press: x:" + mInitialTouchX + ",y:" + mInitialTouchY);
